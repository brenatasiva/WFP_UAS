                    ...
                    <div class="card">
                        ...
                        <div class="card-footer d-flex align-items-center justify-content-between">
                            ...
                            <a class="small text-danger"
                                href="{{ route('product.edit',$d['products']->id) }}">Edit
                                Details</a>
                            <div>
                                <button class="btn btn-danger"
                                        onclick="if(confirm('are you sure delete this record?'))event.preventDefault();
                                                        document.getElementById('delete-form-{{$d['products']->id}}').submit();
                                                ">
                                            {{ __('Delete') }}
                                </button>

                                <form id="delete-form-{{$d['products']->id}}" action="{{ route('product.destroy',$d['products']->id) }}" method="POST" class="d-none">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </div>
